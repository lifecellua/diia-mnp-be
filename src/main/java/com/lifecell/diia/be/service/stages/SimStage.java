...
@Service
public class SimStage implements StageHandler {
    ...
    private static final String TARGET_NAME = "sim";
    ...
    public Response form(Context context) {
        return ResponseForm.builder()
                .form(templateService.generate(TEMPLATE_NAME,
                        TemplateService.mapOf("CANCEL", TemplateService.Operation.KEEP)))
                .build();
    }
    ...
    public Response leave(Context context, Request request) {
        ...
        switch (data.getCommand()) {
            case COMMIT: {
                ...
                var rCode = lifecellService.ckeckIccdAndPuk1(context.getFlow().getId().toString(), context.getSession().getUserRef(), context.getFlow().getMsisdn(), iccid, puk1);
                var rProblem = toProblem(rCode);
                if (rProblem == Problem.SUCCESS) {
                    context.getFlow().setIccid(iccid);
                    return ResponseTransit.builder()
                            .stage(Stage.STAGE_COMMIT)
                            .build();
                } else {
                    if (rProblem.isFatal()) {
                        throw new FsmException(rProblem, "Fail checking sim");
                    }
                    return ResponseNotify.builder()
                            .problem(rProblem)
                            .build();
